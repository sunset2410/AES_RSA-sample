...
String s = "truong dai hoc nach khoa ha noi, dien tu vien thong 12 k54";  //kieu string;
System.out.println(s+"\n");
byte[] bytes = s.getBytes();   //tu kieu string sang kieu byte;
BigInteger message = new BigInteger(bytes); //tu kieu byte sang kieu biginteger;
BigInteger encrypt = key.encrypt(message);   //ma hóa;
BigInteger decrypt = key.decrypt(encrypt);   //giai ma;
System.out.println("public_key"+key.publicKey_e+"\n");
System.out.println("private_key"+key.privateKey_d+"\n");
System.out.println("message = " + message+ "\n");
System.out.println("encrypted = " + encrypt+ "\n");
System.out.println("decrypted = " + decrypt+ "\n");
byte[] byte2=decrypt.toByteArray();    //tu kieu biginteger sang mang byte;
String d= new String(decrypt.toByteArray());  //tu mang byte cho sang string;
