...
public class ClientConnect extends Thread {
	...
	public void run(){
		...
		while(run_flag){
                    ...
                    String publickeyRSA=getMSG(); //lay puclickeyRSA;
			...
			if(checkNickOnLine(nickName)){ // nick nay da co rui.
				...
				server.listUser.put(nickName, this);//add them nick vao listUser;
                                server.ListPublicKey.put(nickName, publickeyRSA); //luu publickeyRSA vao list;
                                server.ListModulusKey.put(nickName,Modulus); //luu modulus vao list;
				...
				while(run_flag){
					...
					switch(stt){
						...
						server.listUser.remove(this.nickName);
                                                server.ListPublicKey.remove(this.nickName); //remove publickeyRSA;
                                                server.ListModulusKey.remove(this.nickName);
						exit_server(); //goi ham exit() de huy ket noi voi client nay dong thoi cac client khac update duoc danh sach online moi.
						break;
						...
						break;
                                            case 2://client muon nho server kiem tra nick chat rieng;    
                                                ...
                                                if(checkNickOnLine(msg))  sendMSG("11",server.ListPublicKey.get(msg),server.ListModulusKey.get(msg));// 11 la ton tai online;    
                                            ...
                                            case 3://client chi muon gui cho 1 nguoi duy nhat;
                                                ...
                                                server.listUser.get(to).sendMSG("7",nickName,message); //gửi message;
                                                
                                                break;
                                            case 13: //server nhan va chuyen key aes;
                                                // server nhan va gui pass di;
                                               String a=getMSG();//lay dia chi nguoi can gui;
                                               String key=getMSG();
                                               server.listUser.get(a).sendMSG("14",nickName,key); 
                                               server.listUser.get(a).sendMSG(server.ListPublicKey.get(nickName)); //gửi khóa publickeyRSA;
                                               server.listUser.get(a).sendMSG(server.ListModulusKey.get(nickName)); //gui modulusRSA;
                                               //nhan va gui tin nhan;
                                               String b=getMSG();//lay ki tu 3;
                                               String c=getMSG();//lay yourname cung chinh la a;
                                               String d=getMSG();//lay noi dung cua message;
                                               server.listUser.get(c).sendMSG("7",nickName,d); 
                                               break;
                                            
                                            case 15: //server nhan yeu cau thoat tu client;
                                               String a1= getMSG(); //my name;
                                               String a2=getMSG(); //youname;
                                               server.listUser.get(a2).sendMSG("16",a1,"1");  
                                               break;
                                            case 20:
                                                //----------recieve file-------------------- 
                                               ...
                                               break;
                                            case 22:
                                                String myname =getMSG();
                                                String yourname =getMSG();
                                                server.listUser.get(myname).sendMSG("23",yourname,"");         
                                                server.listUser.get(myname).sendMSG(server.ListPublicKey.get(yourname)); //gửi khóa publickeyRSA;
                                                server.listUser.get(myname).sendMSG(server.ListModulusKey.get(yourname)); //gui modulusRSA;
                                                break;
